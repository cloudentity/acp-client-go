// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
	"github.com/go-openapi/validate"
)

// OpenbankingBrasilConsentDocument OpenbankingBrasilConsentDocument Document
//
// swagger:model OpenbankingBrasilConsentDocument
type OpenbankingBrasilConsentDocument struct {

	// Nmero do documento de identificao oficial do usurio.
	// Example: 11111111111
	// Required: true
	// Max Length: 11
	// Pattern: ^\d{11}$
	Identification string `json:"identification"`

	// Tipo do documento de identificao oficial do usurio.
	// Example: CPF
	// Required: true
	// Max Length: 3
	// Pattern: ^[A-Z]{3}$
	Rel string `json:"rel"`
}

// Validate validates this openbanking brasil consent document
func (m *OpenbankingBrasilConsentDocument) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateIdentification(formats); err != nil {
		res = append(res, err)
	}

	if err := m.validateRel(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *OpenbankingBrasilConsentDocument) validateIdentification(formats strfmt.Registry) error {

	if err := validate.RequiredString("identification", "body", m.Identification); err != nil {
		return err
	}

	if err := validate.MaxLength("identification", "body", m.Identification, 11); err != nil {
		return err
	}

	if err := validate.Pattern("identification", "body", m.Identification, `^\d{11}$`); err != nil {
		return err
	}

	return nil
}

func (m *OpenbankingBrasilConsentDocument) validateRel(formats strfmt.Registry) error {

	if err := validate.RequiredString("rel", "body", m.Rel); err != nil {
		return err
	}

	if err := validate.MaxLength("rel", "body", m.Rel, 3); err != nil {
		return err
	}

	if err := validate.Pattern("rel", "body", m.Rel, `^[A-Z]{3}$`); err != nil {
		return err
	}

	return nil
}

// ContextValidate validates this openbanking brasil consent document based on context it is used
func (m *OpenbankingBrasilConsentDocument) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *OpenbankingBrasilConsentDocument) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *OpenbankingBrasilConsentDocument) UnmarshalBinary(b []byte) error {
	var res OpenbankingBrasilConsentDocument
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
