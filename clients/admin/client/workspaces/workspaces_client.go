...
func (a *Client) ListUserWorkspaces(params *ListUserWorkspacesParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*ListUserWorkspacesOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewListUserWorkspacesParams()
	}
	op := &runtime.ClientOperation{
		...
		Schemes:            []string{"https"},
		Params:             params,
		Reader:             &ListUserWorkspacesReader{formats: a.formats},
		AuthInfo:           authInfo,
		Context:            params.Context,
		Client:             params.HTTPClient,
	}
	...
	success, ok := result.(*ListUserWorkspacesOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for listUserWorkspaces: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
