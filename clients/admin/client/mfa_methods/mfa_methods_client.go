...
// New creates a new mfa methods API client with basic auth credentials.
...
// New creates a new mfa methods API client with a bearer token for authentication.
// It takes the following parameters:
// - host: http host (github.com).
// - basePath: any base path for the API client ("/v1", "/v3").
// - scheme: http scheme ("http", "https").
...
func (a *Client) TestMFAMethod(params *TestMFAMethodParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*TestMFAMethodOK, error) {
	// TODO: Validate the params before sending
	if params == nil {
		params = NewTestMFAMethodParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "testMFAMethod",
		Method:             "POST",
		PathPattern:        "/mfa-methods/{mfaID}/test",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"https"},
		Params:             params,
		Reader:             &TestMFAMethodReader{formats: a.formats},
		AuthInfo:           authInfo,
		Context:            params.Context,
		Client:             params.HTTPClient,
	}
	...
	success, ok := result.(*TestMFAMethodOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for testMFAMethod: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
