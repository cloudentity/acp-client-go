...
// New creates a new scopes API client with basic auth credentials.
...
// New creates a new scopes API client with a bearer token for authentication.
// It takes the following parameters:
// - host: http host (github.com).
// - basePath: any base path for the API client ("/v1", "/v3").
// - scheme: http scheme ("http", "https").
...
func (a *Client) ListScopesSystem(params *ListScopesSystemParams, authInfo runtime.ClientAuthInfoWriter, opts ...ClientOption) (*ListScopesSystemOK, error) {
	...
	if params == nil {
		params = NewListScopesSystemParams()
	}
	op := &runtime.ClientOperation{
		ID:                 "listScopesSystem",
		Method:             "GET",
		PathPattern:        "/scopes/{wid}",
		ProducesMediaTypes: []string{"application/json"},
		ConsumesMediaTypes: []string{"application/json"},
		Schemes:            []string{"https"},
		Params:             params,
		Reader:             &ListScopesSystemReader{formats: a.formats},
		AuthInfo:           authInfo,
	...
	success, ok := result.(*ListScopesSystemOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for listScopesSystem: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
