...
type AuditEvent struct {
	...
	// action
	// Enum: [authorized unauthorized created updated deleted requested accepted rejected revoked issued denied granted attempted failed sent not_sent]
	...
	// event subject
	// Enum: [request gateway_request gateway_policy policy recovery client login consent authorization_code access_token scopes otp]
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["authorized","unauthorized","created","updated","deleted","requested","accepted","rejected","revoked","issued","denied","granted","attempted","failed","sent","not_sent"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["request","gateway_request","gateway_policy","policy","recovery","client","login","consent","authorization_code","access_token","scopes","otp"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
